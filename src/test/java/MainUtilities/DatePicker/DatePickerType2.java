...
public class DatePickerType2 {
    ...
    String day = "22", month = "January", year = "2026";
    ...
    public void testDatePicker() throws InterruptedException {
        ...
        int expectedYearInt = Integer.parseInt(year); // 2024 set this
        int actualYearInt = Integer.parseInt(yearText); //2025 Ui

        while (actualYearInt != expectedYearInt){
        ...
        int expectedMonthNumber = returnMonthNumber(month);
        int actualMonthNumber = returnMonthNumber(monthText);
        while (expectedMonthNumber != actualMonthNumber){

            // 1, exp 7
            if (actualMonthNumber < expectedMonthNumber){
                nextElement.click();
                monthElement = driver.findElement(By.xpath("//span[contains(@class, 'month')]"));

                monthText = monthElement.getText();
                actualMonthNumber = returnMonthNumber(monthText);
            } else {
                prevElement.click();
                monthElement = driver.findElement(By.xpath("//span[contains(@class, 'month')]"));
